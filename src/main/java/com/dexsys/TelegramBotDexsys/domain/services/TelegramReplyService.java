    ...
    public synchronized void setText(SendMessage sendMessage, UserDTO userDTO) {
        switch(userDTO.getText()) {
            case "Показать профиль":
                User userToShow = dataService.getUser(userDTO.getChatId());
                    sendMessage.setText(
                        ...
                        "Gender: " + (userToShow.isMale() ? "Male" : "Female")+"\n"+
                        ...
                        log.info("Отправлено сообщение о профиле пользователя в Telegram-чат с chatId: {}", userDTO.getChatId());
                break;
            case "Показать пользователей": sendMessage.setText(dataService.getUsers().toString());
                log.info("Отправлено сообщение о профилях всех пользователей в Telegram-чат с chatId: {}", userDTO.getChatId());
                break;
            case "Информация": sendMessage.setText("Кнопка \"Изменить дату рождения\" позволяет " +
                ...
                log.info("Отправлено сообщение об описании интерфейса в Telegram-чат с chatId: {}", userDTO.getChatId());
                break;
            case "Авторизоваться по номеру телефона": sendMessage.setText("Вы авторизованы в системе");
                log.info("Отправлено сообщение об успешной авторизации пользователя в Telegram-чат с chatId: {}", userDTO.getChatId());
                break;
            case "Пользователь не найден":
                sendMessage.setText("Пользователя с телефоном " + userDTO.getPhone() + " нет в базе данных");
                log.info("Отправлено сообщение \"пользователь не найден\" в Telegram-чат с chatId: {}", userDTO.getChatId());
                break;
            default:
                sendMessage.setText(userDTO.getText());
                log.info("Отправлено сообщение \"{}\" в Telegram-чат с chatId: {}", userDTO.getText(), userDTO.getChatId());
        }
    }
    @Override
    ...
    public synchronized void setTextDefault(SendMessage sendMessage, UserDTO userDTO) {
        switch(userDTO.getText()) {
            ...
            case "Очистить профиль": sendMessage.setText("Ваш профиль очищен, вы не авторизованы в системе");
                log.info("Отправлено сообщение об очистке профиля пользователя в Telegram-чат с chatId: {}", userDTO.getChatId());
                break;
            default:
                sendMessage.setText(userDTO.getText());
                log.info("Отправлено сообщение \"{}\" в Telegram-чат с chatId: {}", userDTO.getText(), userDTO.getChatId());
